...
Сейчас `beginners.html` показывает первый ролик из массива `METEOR_VIDEOS` в карточке урока 1. Чтобы заменить видео, измените первый объект в `content/videos.js`.

Для MP4 используется поле `src`, для Rutube - поле `rutube`.
